...
// error. Son situaciones distintas
...
// el store registra acá su función de cierre
...
// El token se agrega en un solo lugar, para toda la aplicación.
  ...
  try {
    ...
    // Solo se entra aqui si NO hubo respuesta del servidor:
    // sin internet, servidor apagado, DNS caído.
    // Un 404 o un 500 NO caen aqui: eso sí es una respuesta.

    if (options.retry === true) {
      // El reintento automático
...
// 204 significa "operación exitosa, sin contenido de respuesta".
  ...
  // A partir de aqui, el servidor respondió con un error.
  ...
  if (status >= 500) {
    // Error en consola para depuración.
